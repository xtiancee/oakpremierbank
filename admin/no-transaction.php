...
$number_transfer = ($_POST['number_transfer']);


$sql=$db->prepare("update settings   set number_transfer=:number_transfer");
$sql->bindParam(':number_transfer',$number_transfer,PDO::PARAM_STR, 15);

if($sql->execute()){

header('location: no-transaction?action=set');
}
else
{
?>
				...
				<section>
					<div class="section-body">
						<div class="row">
							...
							<header class="text-color">Number of Transaction Setup</header>
<br/>
								<div class="card">
				...
				if(isset($_GET['action']) && $_GET['action'] == 'set'){
					...
					echo " <h2 class='w3-small'>Number of transaction successfuly set</h2></div> ";
				}
				?>               
								 <?php 
...
foreach($db->query($sqla, PDO::FETCH_ASSOC) as $row)
	...
	?>
        ...
        <div class="modal-body">
          <p>Setup Number of Transaction per User.</p>
		  <div class="container-fluid">
		  <div class="row">
			<div class="col-md-12">
			<div class="form-group">
				<label>Enter Number</label>
				<input type="text" class="form-control" name="number_transfer" placeholder="eg 4 times" value="<?php echo $row['number_transfer'] ; ?>" required>
			</div>
			</div>
